...
public:
	...
	void setTheProjectionMatrix(float left, float right, float bottom, float top, float nearCP, float farCP);
	glm::mat4 getTheProjectionMatrix();
	glm::mat4 getTheViewMatrix();

	void moveTheCameraForward(float distance);
	void moveTheCameraLeft(float distance);
	void moveTheCameraRight(float distance);
	void moveTheCameraBack(float distance);
